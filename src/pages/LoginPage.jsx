...
export default function LoginPage() {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch({ type: ACTIONS.LOGIN_USER, payload: { name: email.split('@')[0] } })
  ...
  return (
    <Box className={`login-root ${state.theme === 'dark' ? 'dark-theme' : ''}`}>
      <AppBar position="absolute" sx={{ bgcolor: 'transparent', boxShadow: 'none', top: 16, left: 16, right: 'auto' }}>
        <Toolbar sx={{ padding: 0, display: 'flex', gap: 1 }}>
          <Button
            ...
            sx={{
              color: 'white',
              bgcolor: state.theme === 'light' ? 'rgba(25, 118, 210, 0.8)' : 'rgba(255,255,255,0.2)',
              px: 1.5,
              py: 0.5,
              borderRadius: 1,
              textTransform: 'none',
              ...
              '&:hover': {
                bgcolor: state.theme === 'light' ? 'rgba(25, 118, 210, 1)' : 'rgba(255,255,255,0.3)',
